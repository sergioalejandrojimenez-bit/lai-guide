...
const TOC = ({ searchQuery }) => {
  return (
    <>
      ...
      <StepAccordion stepNum="TC" title="Estándar TC 1000 ppm (KHP)" machineCode="toc" searchQuery={searchQuery}>
        <SubItemCheck title={<>Pesar 0.2125 g de KHP (secado a 120°C × 1 hora)</>}>
        ...
        <SubItemCheck title={<>Transferir a balón 100 mL, agregar agua, ultrasonido, aforar</>}>
          <p>Usar agua Milli-Q. Ultrasonido por 15 min. Dejar enfriar antes de aforar a 100 mL = 1000 ppm.</p>
        </SubItemCheck>
        <SubItemCheck title={<>Tomar 1.5 mL → balón de 25 mL → aforar = 60 ppm TC</>}>
          <p>Esta es la solución de trabajo para inyectar. El equipo diluye automáticamente para la curva a partir de estos 60 ppm.</p>
        </SubItemCheck>
      </StepAccordion>

      <StepAccordion stepNum="IC" title="Estándar IC 1000 ppm (NaHCO₃ + Na₂CO₃)" machineCode="toc" searchQuery={searchQuery}>
        <SubItemCheck title={<>Pesar 0.35 g NaHCO₃ + 0.441 g Na₂CO₃ (secado 290°C × 1 h)</>}>
          <p>El Na₂CO₃ requiere mayor temperatura para eliminar agua de cristalización.</p>
        </SubItemCheck>
        <SubItemCheck title={<>Mismo procedimiento físico que el TC para obtener 60 ppm IC</>}>
          <p>Balón 100 mL → 1.5 mL a balón de 25 mL = 60 ppm IC.</p>
        </SubItemCheck>
      </StepAccordion>

      <h3 className="sec-title">Flujo de trabajo completo</h3>
      <StepAccordion stepNum="⚙" title="Operación en Sistema (TOC-V)" machineCode="toc" searchQuery={searchQuery}>
        <SubItemCheck title={<>Crear Curvas TC e IC desde 60 ppm</>}>
          <p>TOC-V Sample Table Editor → Cal. Curve → New. Parametros: Múltiples Inyecciones, CV {'<'} 2.0%, dilución de estándar.</p>
        </SubItemCheck>
        <SubItemCheck title={<>Crear Tabla de Muestras e Iniciar</>}>
