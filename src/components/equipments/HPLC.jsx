...
const HPLC = ({ searchQuery }) => {
  return (
    <>
      ...
      <StepAccordion stepNum="1" title="Encendido y Columna" machineCode="hplc" searchQuery={searchQuery}>
        <SubItemCheck title={<>Filtrar solventes y verificar columna instalada</>}>
        ...
        <SubItemCheck title={<>Orden CRÍTICO: Encender Equipo HPLC ANTES de abrir LCsolution</>}>
          <div className="dang">🔴 El software LCsolution NO reconoce el equipo si se enciende después del programa.</div>
        </SubItemCheck>
      </StepAccordion>

      <StepAccordion stepNum="2" title="Configuración de Método y Análisis" machineCode="hplc" searchQuery={searchQuery}>
        <SubItemCheck title={<>Instrument Parameters View (Oven, Wavelength, Flow)</>}>
          <p>Configurar temperatura de horno, longitud de onda en Detector A (ej. 254nm), flujo y composición de bomba.</p>
        </SubItemCheck>
        <SubItemCheck title={<>Clic en Download para enviar al hardware</>}>
          <p>El botón Download transfiere los parámetros e inicia el ciclado del solvente.</p>
        </SubItemCheck>
        <SubItemCheck title={<>Single Run cuando la línea base esté plana</>}>
          <p>Clickea Single run, ingresa Vial#, Tray#, Sample ID e Injection Volume. Luego OK para start.</p>
        </SubItemCheck>
      </StepAccordion>

      <StepAccordion stepNum="3" title="Troubleshooting Básico" machineCode="hplc" searchQuery={searchQuery}>
        <div className="tc">
